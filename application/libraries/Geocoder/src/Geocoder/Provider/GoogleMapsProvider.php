...
class GoogleMapsProvider extends AbstractProvider implements LocaleAwareProviderInterface
{
    ...
    protected function executeQuery($query)
    {
        ...
        //error_log(__METHOD__ . "(): CONTENT: {$content}");
        ...
        if (null === $content) {
            throw new NoResultException(sprintf('1: Could not execute query %s', $query));
        }

        $json = json_decode($content);
        //error_log(__METHOD__ . "(): JSON:" . print_r($json, 1));

        // API error
        if (!isset($json)) {
            throw new NoResultException(sprintf('2: Could not execute query %s', $query));
        }
        ...
        if (!isset($json->results) || !count($json->results) || 'OK' !== $json->status) {
            throw new NoResultException(sprintf('3: Could not execute query %s', $query));
        }
        ...
        //error_log(__METHOD__ . "(): FINAL RESULT: " . json_encode($results));
        return $results;
    }
